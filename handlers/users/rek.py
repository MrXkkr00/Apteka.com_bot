...
#     db.add_user_rek(1312312)
#     db.add_user_rek(1432412)
#     db.add_user_rek(131245312)
#
#     users = db.select_all_users_rek()
#     print(f"Barcha fodyalanuvchilar: {users}")
#
#     user = db.select_user_rek(id=2)
#     print(f"Bitta foydalanuvchini ko'rish: {user}")
#     sum = db.count_users_rek()
#     print(sum)
#
#
# test()
...
from keyboards.default.menu import bosh_menu, bosh_menu_IN
...
async def bot_s1213tart(message: types.Message):
    ...
    if not user:
        ...
        f.write(f"{user_id}\n")
        f.close()
    await message.answer(f"Здравствуйте и добро пожаловать в наш бот", reply_markup=bosh_menu)
    await message.answer("Вы можете воспользоваться нашими услугами", reply_markup=bosh_menu_IN)
    await bot.send_message(6975551943, F"@{message.from_user.username} Botga kirdi")
    await bot.send_message(984568970, F"@{message.from_user.username} Botga kirdi")
...
async def bot22_start(message: types.Message):
    ...
    for i in range(sum):
        user = db.select_user_rek(id=i + 1)
        await bot.send_message(user[1], f"{user[0]}    {user[1]}   @{user[2]}")
